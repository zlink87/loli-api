...
| `conditioning` | CONDITIONING | 아니요 | - | CLIPTextEncode의 텍스트 컨디셔닝. 텍스트 프롬프트를 사용한 탐지 시 필요 |
...
| `positive_coords` | STRING | 아니요 | - | 픽셀 좌표를 사용한 JSON 형식 `[{"x": int, "y": int}, ...]`의 양성 포인트 프롬프트. 분할에 포함하려는 포인트 |
| `negative_coords` | STRING | 아니요 | - | 픽셀 좌표를 사용한 JSON 형식 `[{"x": int, "y": int}, ...]`의 음성 포인트 프롬프트. 분할에서 제외하려는 포인트 |
...
| 출력 이름 | 데이터 타입 | 설명 |
|-------------|-----------|-------------|
| `masks` | MASK | 분할 마스크. `individual_masks`가 False(기본값)인 경우 프레임당 단일 결합 마스크를 반환합니다. True인 경우 감지된 각 객체에 대한 개별 마스크를 반환합니다 |
