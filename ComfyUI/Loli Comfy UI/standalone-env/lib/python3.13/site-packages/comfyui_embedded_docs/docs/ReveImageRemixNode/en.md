...
| `prompt` | STRING | Yes | 1 to 2560 characters | A text description of the desired image. You can include XML `<img>` tags to reference specific images by their index (e.g., `<img>0</img>`, `<img>1</img>`). |
...
| `seed` | INT | No | 0 to 2147483647 | A seed value. Changing this value will cause the node to re-run, but the results are non-deterministic. (default: 0) |

**Note:** The `model` parameter is a dynamic combo that includes nested settings for `aspect_ratio` (e.g., "auto", "16:9", "1:1") and `test_time_scaling`. The `upscale` parameter, when set to "enabled", reveals a nested `upscale_factor` setting.
