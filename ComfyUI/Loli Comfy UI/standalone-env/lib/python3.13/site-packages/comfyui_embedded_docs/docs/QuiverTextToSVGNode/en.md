...
| `reference_images` | IMAGE | No | N/A | Up to 4 reference images to guide the generation. This is an optional input. |
| `model` | COMBO | Yes | Multiple options available | Model to use for SVG generation. The available options are determined by the Quiver API. |
