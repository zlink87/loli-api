...
def test_deterministic_respects_per_field_voice():
    ...
    # 2nd person, addressed to the AI
    assert values["system_prompt"].startswith("You are Nora,")
    assert "Never break character" in values["system_prompt"]
...
def test_venice_all_fields_present_is_provider_venice():
    w = _writer(api_key="k")
    w._client.chat = _fake_chat('{"system_prompt":"You are Nora.","tone":"bold, teasing"}')
...
def test_venice_partial_fills_missing_and_is_mixed():
    w = _writer(api_key="k")
    w._client.chat = _fake_chat('{"system_prompt":"You are Nora."}')
    values, provider = asyncio.run(w.write(_persona(), ["system_prompt", "tone"], {}, name="Nora"))
    assert values["system_prompt"] == "You are Nora."
