...
class BackgroundEditWorker(BaseEditWorker):
    ...
    async def _process_job(self, job: Job) -> None:
        ...
        try:
            ...
            # Step 5: Get output images (same output node as outfit: 116)
            images = (
                output_images.get("116") if isinstance(output_images, dict) else None
            )
            if not images:
                images = (
                    next(iter(output_images.values()), []) if output_images else []
                )
            if not images:
            ...
            relative_path, preview_url, expires_at, image_hash = (
                await self.save_output_image(image_data, job.job_id, "background_edits")
            )
